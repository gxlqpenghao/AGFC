...
def test_repair_mineru_artifact_writes_repaired_bundle_without_mutating_original(tmp_path: Path):
    ...
    assert repaired_items[1]["asset_path"] == "../postprocessed/final_images/0000_page_000_figure_1.png"
...
def test_repair_mineru_artifact_does_not_fallback_to_different_page_asset(tmp_path: Path):
    source_path = tmp_path / "source.pdf"
    source_path.write_bytes(b"%PDF-demo\n")
    artifact_dir = tmp_path / "mineru"
    artifact_dir.mkdir()
    original_content = [
        {"type": "image", "page_idx": 12, "bbox": [0, 0, 10, 10], "asset_id": "page12_first"},
        {"type": "image", "page_idx": 12, "bbox": [20, 20, 30, 30], "asset_id": "page12_second"},
    ]
    (artifact_dir / "content_list.json").write_text(json.dumps(original_content), encoding="utf-8")
    ...
    extract_images = extract_run_dir / "images"
    extract_images.mkdir(parents=True)
    (extract_images / "page_012_panel_1.png").write_bytes(b"same-page-image")
    (extract_images / "page_009_fallback_1.png").write_bytes(b"different-page-image")
    extract_result = {
        "engine": "agfc",
        "engine_version": "0.1.0",
        "artifacts": {"run_dir": str(extract_run_dir), "images_dir": str(extract_images)},
        "images": [
            {
                "page_idx": 12,
                "asset_id": "page_012_panel_1",
                "asset_path": "images/page_012_panel_1.png",
            },
            {
                "page_idx": 9,
                "asset_id": "page_009_fallback_1",
                "asset_path": "images/page_009_fallback_1.png",
            },
        ],
    }
    ...
    assert repaired_items[0]["asset_id"] == "page_012_panel_1"
    assert repaired_items[1]["asset_id"] == "page12_second"
    assert result["replacements"][1]["decision"] == "keep_original"
